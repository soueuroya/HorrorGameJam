...
public class LTDescr
{
	...
	public void reset(){
		...
		this.hasUpdateCallback = this.useEstimatedTime = this.useFrames = this.hasInitiliazed = this.onCompleteOnRepeat = this.destroyOnComplete = this.onCompleteOnStart = this.useManualTime = this.hasExtraOnCompletes = this.hasExtraOnCancels = false;
...
//		Debug.Log ("tween:" + this+ " dt:"+dt);
	...
	public void callOnCancels()
	{
		...
		if (this.type == TweenAction.DELAYED_SOUND)
		{
			AudioSource.PlayClipAtPoint((AudioClip)this._optional.onCancelParam, this.to, this.from.x);
		}
		if (this._optional.onComplete != null)
		{
			this._optional.onCancel();
		}
		else if (this._optional.onCancelObject != null)
		{
			this._optional.onCancelObject(this._optional.onCancelParam);
		}
	}
...
//		val = (val * val * val * val - 2f);
	...
	*/
	...
	/**
* Have a method called when the tween cancels
* @method setOnComplete
* @param {Action} onCancel:Action the method that should be called when the tween is canceled ex: tweenCanceled(){ }
* @return {LTDescr} LTDescr an object that distinguishes the tween
* @example
* LeanTween.moveX(gameObject, 5f, 2.0f ).setOnCancel( tweenCaneled);
*/
	public LTDescr setOnCancel(Action onCancel)
	{
		this._optional.onCancel = onCancel;
		this.hasExtraOnCancels = true;
		return this;
	}

	/**
	* Have a method called when the tween cancels
	* @method setOnCancel (object)
	* @param {Action<object>} onCancel:Action<object> the method that should be called when the tween is canceled ex: tweenCanceled( object myObj ){ }
	* @return {LTDescr} LTDescr an object that distinguishes the tween
	* @example
	* object tweenCanceledObj = "hi" as object;
	* LeanTween.moveX(gameObject, 5f, 2.0f ).setOnCancel( tweenCanceled, tweenCanceledObj );
	*/
	public LTDescr setOnCancel(Action<object> onCancel)
	{
		this._optional.onCancelObject = onCancel;
		this.hasExtraOnCancels = true;
		return this;
	}
	public LTDescr setOnCancel(Action<object> onCancel, object onCancelParam)
	{
		this._optional.onCancelObject = onCancel;
		this.hasExtraOnCancels = true;
		if (onCancelParam != null)
			this._optional.onCancelParam = onCancelParam;
		return this;
	}

	/**
	* Pass an object to along with the onCancel Function
	* @method setOnCancelParam
	* @param {object} onCancel:object an object that 
	* @return {LTDescr} LTDescr an object that distinguishes the tween
	* @example
	* LeanTween.delayedCall(1.5f, enterMiniGameStart).setOnCancelParam( new object[]{""+5} );<br><br>
	* void enterMiniGameStart( object val ){<br>
    * &nbsp;object[] arr = (object [])val;<br>
    * &nbsp;int lvl = int.Parse((string)arr[0]);<br>
    * }<br>
	*/
	public LTDescr setOnCancelParam(object onCancelParam)
	{
		this._optional.onCancelParam = onCancelParam;
		this.hasExtraOnCancels = true;
		return this;
